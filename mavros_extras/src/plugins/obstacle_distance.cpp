	...
	namespace extra_plugins
	{
		...
		public:
			...
			void initialize(UAS &uas_) override
			{
				...
				target_system = m_uas->get_tgt_system();

				laserTopic = "/" + std::to_string(target_system) + "/laserScan";

				ROS_INFO_STREAM("Laser Scan Topic Name: " << laserTopic);

				obstacle_sub = obstacle_nh.subscribe(laserTopic, 10, &ObstacleDistancePlugin::obstacle_cb, this);
			}
